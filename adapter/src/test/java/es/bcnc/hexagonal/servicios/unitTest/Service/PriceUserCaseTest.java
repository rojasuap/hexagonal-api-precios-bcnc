package es.bcnc.hexagonal.servicios.unitTest.Service;
...
class PriceUserCaseTest {
    ...
    @InjectMocks
    private GetPriceByCurrentDateAndProductIdAndBrandIdService getPriceByCurrentDateAndProductIdAndBrandIdService;

    @DisplayName("Test General UserCase: Petición a las 10:00 del día 14 del producto 35455   para la brand 1 (ZARA)")
    @Test
    @Order(1)
    void testServiceGetPricesByCurrentDateAndProductIdAndBrandIdGeneral(){
        ...
        //given
        given(priceRepository.getPricesByCurrentDateAndProductIdAndBrandId(currentDateTime,PRODUCT_ID,BRAND_ID)).willReturn(Optional.of(priceModel));

        //when: Acción para probar
        Optional<PriceModel> priceModelNuevo = getPriceByCurrentDateAndProductIdAndBrandIdService.getPricesByCurrentDateAndProductIdAndBrandId(currentDate,PRODUCT_ID,BRAND_ID);
